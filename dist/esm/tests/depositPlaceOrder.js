...
export async function depositPlaceOrder(connection, payerKeypair, marketAddress, numBaseTokens, tokenPrice, isBid, orderType, clientOrderId, minOutTokens = 0, lastValidSlot = 0) {
    const client = await ManifestClient.getClientForMarket(connection, marketAddress, payerKeypair);
    const depositPlaceOrderIx = client.placeOrderWithRequiredDepositIx(payerKeypair.publicKey, {
        numBaseTokens,
        tokenPrice,
        isBid,
