import { ManifestClient, RestingOrder } from '@cks-systems/manifest-sdk';
import { createBatchUpdateInstruction, OrderType } from '@cks-systems/manifest-sdk/manifest';
...
const keypair: Keypair = Keypair.fromSecretKey(Uint8Array.from(process.env.PRIVATE_KEY!.split(',').map(Number)),);
...
const run = async () => {
    ...
    for (const openOrder of openOrders) {
        if (openOrder.orderType == OrderType.Reverse && openOrder.trader.toBase58() == keypair.publicKey.toBase58()) {
