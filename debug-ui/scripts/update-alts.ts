...
import { AddressLookupTableAccount, AddressLookupTableProgram, Connection, Keypair, MessageV0, PublicKey, Transaction, VersionedTransaction } from '@solana/web3.js';
import { Pool } from 'pg';
...
async function findMarketsWithoutAlt(connection: Connection, pool: Pool): Promise<string[]> {
    const allMarketPks = new Set((await ManifestClient.listMarketPublicKeys(connection)).map(pk => pk.toString()));
    const altQueryResult = await pool.query(
        'SELECT market from alt_markets'
    );
...
async function addMarketToAlts(connection: Connection, pool: Pool, keypair: Keypair, market: string) {
    ...
    const altsWithSufficientSpace = await pool.query(
        'SELECT * from alts WHERE remaining_space > $1 ORDER BY remaining_space ASC', [spaceNeeded]
    )
    ...
    for (const row of altsWithSufficientSpace.rows) {
        ...
        const ix = AddressLookupTableProgram.extendLookupTable({
            ...
            lookupTable: altPk,
            addresses: pksToAdd.values().map(pk => new PublicKey(pk)).toArray(),
        });
        ...
        console.log('add market', market, 'to existing alt', altAddress, 'remaining_space', row.remaining_space, '->', remaingSpace);
        ...
        await pool.query(
            'INSERT INTO alt_markets(alt, market) VALUES ($1, $2)',
            [altAddress, market]
        )
    ...
    let extendIx = AddressLookupTableProgram.extendLookupTable({
        ...
        authority: keypair.publicKey,
        lookupTable: altPk,
        addresses: pksForSwap.values().map(pk => new PublicKey(pk)).toArray(),
    })
    const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash('finalized');
    const tx = new VersionedTransaction(
        MessageV0.compile({
