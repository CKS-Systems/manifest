...
export class LiquidityMonitor {
  ...
  calculateMarketMakerDepths(market: Market): MarketMakerStats[] {
    ...
    for (const trader of traders) {
      const traderBids = bids.filter(order => 
        order.trader.toBase58() === trader
      );
      const traderAsks = asks.filter(order => 
        order.trader.toBase58() === trader
      );
  ...
  async monitorMarkets(): Promise<void> {
        ...
        for (const stat of marketStats) {
          for (const spreadBps of SPREAD_BPS) {
            marketMakerDepth.set(
              { market: marketPk, trader: stat.trader, side: 'bid', spread_bps: spreadBps.toString() },
              stat.bidDepth[spreadBps] || 0
            );
            marketMakerDepth.set(
              { market: marketPk, trader: stat.trader, side: 'ask', spread_bps: spreadBps.toString() },
              stat.askDepth[spreadBps] || 0
            );
          }
        }

        console.log(`Processed ${marketStats.length} market makers for market ${marketPk}`);
      } catch (error) {
    ...
    console.log(`Monitoring cycle complete. Processed ${allStats.length} total market maker entries.`);
  }
  ...
  async saveStatsToDatabase(stats: MarketMakerStats[]): Promise<void> {
    ...
    try {
      ...
      const marketInfoValues = Array.from(this.marketInfo.values()).flatMap(info => [
        info.address,
        new Date(),
        info.volume24hUsd,
        info.lastPrice,
      ...
      if (marketInfoValues.length > 0) {
        const marketInfoPlaceholders = Array.from(this.marketInfo.values()).map((_, index) => {
