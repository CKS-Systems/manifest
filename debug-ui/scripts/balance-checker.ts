...
const run = async () => {
  ...
  for (const marketPk of marketPks) {
    ...
    if (
      ...
      // Skip small differences because of javascript rounding issues.
      if (Math.abs(baseExpectedAtoms - baseVaultBalanceAtoms) <= 1 &&
  ...
  if (foundMismatch) {
    throw new Error('Mismatch of expected vs actual');
  }
};
